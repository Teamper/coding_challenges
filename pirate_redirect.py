...
def answer(numbers):
    ...
    def ask_pirate(to_ask,pirates,visited):
        visited[to_ask] += 1
        return {'to_ask':pirates[to_ask],'visited': visited}
    
    
    def count_loop(visited):
        n = 0
        for e in visited:
            if e > 1:
                n += 1
    ...
    pirates_visited = [0] * 5000
    n_loops = 0
    
    results = ask_pirate(start_index,numbers,pirates_visited)
    pirates_visited = results['visited']
    
    while True:
        if pirates_visited[results['to_ask']] > 1:
            break
        results = ask_pirate(results['to_ask'],numbers,pirates_visited)
        pirates_visited = results['visited']
    
    return count_loop(pirates_visited)
